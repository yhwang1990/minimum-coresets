...
//    }
    ...
    static void loss_distribution(const vector<Point> &points, const vector<Point> &queries,
                                  const vector<double> &results, vector<double> &regret_ratios,
                                  vector<double> &regret_values) {
        assert(queries.size() <= results.size());
        int n = queries.size();
        regret_ratios.reserve(n);
        regret_values.reserve(n);
        ...
        for (int i = 0; i < n; i++) {
            ...
            double regret_value_i = results[i] - dot_max;

            regret_ratios[i] = regret_ratio_i;
            regret_values[i] = regret_value_i;
        }
    }
